...
def tool_get_user_costs(user_id: int, db: Session) -> dict:
    ...
    return {
        ...
        "pricing_tier": "Gemini 3.6 Flash + text-embedding-004 (₹6.25 / 1M tokens)",
...
AGENT_SYSTEM_INSTRUCTION = (
    ...
    "1. Natural Conversational Tone: When greeted, respond warmly and strategically. NEVER dump raw JSON.\n"
    "2. Dynamic Deadlines (CRITICAL): If the user asks about deadlines within a specific timeframe (e.g., 'next 3 days', 'this week', 'next month'), YOU MUST mathematically convert that to days and pass it dynamically to the `check_deadlines` tool. Do NOT default to 14 days if the user specified a range.\n"
    "3. Role-Specific Skills: If the user asks 'What skills do I need for role X?', DO NOT just give generic advice. First, use `search_job_listings` or `nl2sql_database_search` to query that specific role in the database. Read the required skills from the returned data, and present them clearly to the user.\n"
    "4. Complex Geo-Semantic Searches: If the user asks for hybrid constraints like 'AI ML job near London', use `search_job_listings` with the full query, OR use `nl2sql_database_search` if strict location filtering is needed. Synthesize the results naturally.\n"
    "5. Resume Matching: When the user asks for roles matching their resume, invoke `match_resume_to_jobs`.\n"
    "6. NL2SQL Presentation Rules: When presenting results from `nl2sql_database_search`, use a clean Markdown table (Role | Company | Location | Deadline | Stipend) and conclude with 2-3 strategic takeaways.\n"
    "7. Contextual Synthesis: Translate tool outputs into clear, authoritative, actionable career advice."
)
...
def agent_chat(
    ...
    def check_deadlines(days: int = 14) -> dict:
        """
        Checks job listings with approaching application deadlines. 
        CRITICAL: If the user specifies a timeframe (e.g., 'next 3 days', 'this week'), pass that exact integer as the 'days' parameter.
        """
    ...
    def search_job_listings(query: str) -> dict:
        ...
        Use this for complex queries combining role and location (e.g., 'AI ML job near London') or to find specific roles to inspect their required skills.
        """
    ...
    if not final_response_text:
        ...
        if any(w in lower_msg for w in ["cost", "token", "bill", "usage", "inr", "spend", "balance"]):
            ...
            # Extract number of days if the user provided one (e.g., "next 3 days")
            days_match = re.search(r'\b(\d+)\s*(days|weeks|months)\b', lower_msg)
            days_to_check = 14 # Default
            if days_match:
                num = int(days_match.group(1))
                unit = days_match.group(2)
                if unit == 'weeks': num *= 7
                elif unit == 'months': num *= 30
                days_to_check = num

            res = check_deadlines(days=days_to_check)
            executed_tool_calls.append(AgentToolCallInfo(tool_name="check_deadlines", tool_input={"days": days_to_check}, tool_output=res))
            ...
            if closing:
                ...
                final_response_text = (
                    ...
                    f"I identified **{len(closing)}** high-priority role(s) closing within the next {days_to_check} days:\n\n"
                    f"{items_text}\n\n"
                    f"Would you like me to inspect required skills or draft an accelerated briefing for any of these?"
                )
            else:
                final_response_text = f"You don't have any tracked jobs closing within the next {days_to_check} days."
